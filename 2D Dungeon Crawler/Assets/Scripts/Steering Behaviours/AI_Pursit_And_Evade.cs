...
public class AI_Pursit_And_Evade : MonoBehaviour
{
    // twerkable
    ...
    [Header("Pursit and Evade Twerkable")]
    public bool isPursit;
    public Transform targetTransform;
    public AI_Seek_Only ai_seek_cs;
    ...
    private void Update()
    {
        ...
        //---------------------------------------
        if(isPursit)
            acceleration += Pursit(ref velocity);
        else
            acceleration += Evade(ref velocity);
    ...
    Vector3 Pursit(ref Vector3 vel)
    {
        Vector3 distance = targetTransform.position - transform.position;
        int time = (int)(distance.magnitude / maxSpeed);
        Vector3 futurePos = targetTransform.position + ai_seek_cs.velocity * time;
        targetPos = futurePos;
        return Seek(ref vel);
    }

    Vector3 Evade(ref Vector3 vel)
    {
        Vector3 distance = targetTransform.position - transform.position;
        int time = (int)(distance.magnitude / maxSpeed);
        Vector3 futurePos = targetTransform.position + ai_seek_cs.velocity * time;
